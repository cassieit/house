...
<head>
    ...
    <script type="text/javascript">
        ...
        //房源图片
        function image() {
            var btn=document.getElementById("ima");
            // var op1=document.getElementsByName("imae");
            $.getJSON("<%=basePath%>imagesController/queryByHousesId",{houses_id:btn.value},function(data){
                var str="";
                for(var i=0;i<data.length;i++) {
                    str=str+"<img   width='286px' height='188px' src='<%=basePath%>images/houses/"+data[i].images_name+"'  />"
    ...
    <script>
        $(document).ready(function() {
            $.getJSON("http://localhost:8892/imagesController/queryByHousesid?houses_id=1",function(data){
                    for (var i = 0; i < data.length; i++) {
                        str = str + "<img src='<%=basePath%>images/houses/"+data[i].images_name+"' >"
                    }
                    $("#aa").jsp(str);

                });
            });
        //E:\IdeaProjects\eurekaClent_Call_feign_silence\src\main\webapp\images\houses\btp_1.jpg
    </script>
    <script>
        $(document).ready(function(){
...
<div class="content">
    <div class="width1190" style="width:1000px;">
        ...
        <div class="proText fr" style="border: 0px">
            ...
            <div class="xun-car">
                ...
                <c:if test="${sessionScope.loginUser==null}">  <a href="<%=basePath%>login.jsp" class="projrgwc">预定请先登录</a></c:if>
                <c:if test="${sessionScope.loginUser!=null}"><a href="http://localhost:8892/orderController/addOrder?users_id=${sessionScope.loginUser.user_id}&houses_id=${sessionScope.houses.houses_id}" class="projrgwc">预定房源</a></c:if>
            </div><!--xun-car/-->
        </div><!--proText/-->
        <div class="clears"></div>
    </div><!--width1190/-->
    <div class="proBox" style="width:1000px;margin:10px auto;">
        <div class="proEq">
            <ul class="fl">
                ...
                <li ><button id="ima" name="imae" type="hidden" value="${sessionScope.houses.houses_id}"  onclick="image()" style="width: 168px;
    height: 50px; outline: none; font-size:20px;border: 0px">房源图片</button></li>
                <%--onclick="image()"--%>
                <li>小区介绍</li>
            </ul>
        ...
        <div class="proList" id="houseImage">
